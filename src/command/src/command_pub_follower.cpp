...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber setformation_sub = nh.subscribe<state_msg::form>("/set_formation", 10, form_cb);
    //【发布】控制指令
    //setpoint_raw_local_pub = nh.advertise<mavros_msgs::PositionTarget>("/setpoint_raw/local", 10);
    //【发布】控制指令
    // 发送-->px4
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>主  循  环<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    while(ros::ok())
    {
        ...
        {
            Command_Now.pose.position.x = Command_Now.pose.position.x - 2;
            Command_Now.pose.position.y = Command_Now.pose.position.y - 1;
       ...
       else if(controller_flag == 1)
        {
            ...
            vel_des[0] = - yita * k_aij * ( vel_nei[0][0] - k_gamma *((pos_drone[0] - pos_nei[0][0]) - ( formation_separation(uav_id-1,0) -  formation_separation(neighbour_id1-1,0)))) 
                         ...
                         + yita * k_p * ( initial_velocity[0] - k_gamma * (pos_drone[0] - Command_Now.pose.position.x - formation_separation(uav_id-1,0)));
            vel_des[1] = - yita * k_aij * ( vel_nei[0][1] - k_gamma *((pos_drone[1] - pos_nei[0][1]) - ( formation_separation(uav_id-1,1) -  formation_separation(neighbour_id1-1,1)))) 
                         ...
                         + yita * k_p * ( initial_velocity[1] - k_gamma * (pos_drone[1] - Command_Now.pose.position.y - formation_separation(uav_id-1,1)));
            vel_des[2] = 0.0;
            acc_des << 0.0, 0.0, 0.0;
